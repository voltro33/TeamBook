function fetchLastFiveMatches(teamId, numMatches) {
    const url = `https://cors-anywhere.herokuapp.com/?https://api.football-data.org/v2/teams/${teamId}/matches?status=FINISHED&limit=${numMatches}`;
    const headers = {
...
function findImage(teamID) {
    return fetch(`https://cors-anywhere.herokuapp.com/?https://api.football-data.org/v2/teams/${teamID}`, {
        headers: {
